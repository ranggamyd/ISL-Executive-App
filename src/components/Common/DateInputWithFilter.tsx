...
interface DateInputWithFilterProps {
    date: string;
...
const DateInputWithFilter: React.FC<DateInputWithFilterProps> = ({ date, setDate, filterRef, filterOpen, setFilterOpen, clearFilters, children }) => {
    const { t } = useLanguage();
    ...
    return (
        <div className="flex justify-center items-center space-x-4">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative">
                <input type="date" value={date} onChange={e => setDate(e.target.value)} className="px-3 py-3 dark:[color-scheme:dark] border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-gray-900 dark:text-white" />
            </motion.div>
...
export default DateInputWithFilter;
