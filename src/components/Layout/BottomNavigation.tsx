...
const BottomNavigation: React.FC = () => {
    ...
    const renderButton = (key: string, iconName: string, label: string, path: string, activeCheckPath?: string) => {
        ...
        return (
            <button key={key} onClick={() => navigate(path)} className="flex-1 flex flex-col items-center justify-center relative py-2 overflow-hidden">
                {active && (
                    <>
                        <motion.span layoutId="active-indicator" className="absolute top-0 w-full h-1 bg-blue-500 rounded-b-md z-30" transition={{ type: "spring", stiffness: 500, damping: 30 }} />
                        <div className="absolute top-0 left-0 right-0 bottom-0 z-10 pointer-events-none bg-gradient-to-b from-blue-200/50 via-blue-200/10 to-transparent blur-sm" />
                    </>
                )}

                <div className="relative z-20 flex flex-col items-center justify-center pt-1">
                    <Icon size={20} className={`mb-1 ${active ? "text-blue-500" : "text-gray-400"} transition-colors duration-200`} />
                    <span className={`text-xs font-medium ${active ? "text-blue-500" : "text-gray-500"} transition-colors duration-200`}>{label}</span>
                </div>
            </button>
        );
    };
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-10">
            <div className="bg-white pb-1 border-t border-gray-200 shadow-md rounded-t-xl">
