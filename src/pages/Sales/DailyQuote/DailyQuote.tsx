    ...
    const salesStats = [
        {
            ...
            value: filteredQuotes.reduce((acc, recap) => acc + parseInt(recap.pelanggan_baru.toString()), 0),
            icon: UserPlus,
        ...
        },
        {
            ...
            value: filteredQuotes.reduce((acc, recap) => acc + parseInt(recap.pelanggan_lama.toString()), 0),
            icon: Users,
        ...
        },
        {
            ...
            value: filteredQuotes.reduce((acc, recap) => acc + parseInt(recap.total_request_quotation.toString()), 0),
            icon: FileText,
        ...
        {
            title: t("newAmount"),
            value: filteredQuotes.reduce((acc, recap) => acc + parseInt(recap.total_biaya_pelanggan_baru.toString()), 0),
            icon: Coins,
        ...
        },
        {
            ...
            value: filteredQuotes.reduce((acc, recap) => acc + parseInt(recap.total_biaya_pelanggan_lama.toString()), 0),
            icon: Wallet,
        ...
        {
            title: t("grandTotal"),
            value: filteredQuotes.reduce((acc, recap) => acc + parseInt(recap.total_biaya_akhir.toString()), 0),
            icon: BarChart3,
            ...
            {loading ? (
                ...
                <div className="grid gap-4">
                    {filteredQuotes.map((recap, index) => (
                        <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center space-x-4">
                                    ...
                                    <div>
                                        <h3 className="font-semibold text-gray-900">{recap.sales_name}</h3>
                                        <div className="space-y-0">
                                            <p className="text-sm text-gray-600 flex items-center">
                                                <small className="font-semibold text-xs text-gray-700 w-20">{t("supervisor")}</small>
                                                <small className="truncate text-xs">: {recap.supervisor.nama_lengkap !== recap.sales_name ? recap.supervisor.nama_lengkap : "-"}</small>
                                            </p>
                                            <p className="text-sm text-gray-600 flex items-center">
                                                <small className="font-semibold text-xs text-gray-700 w-20">{t("manager")}</small>
                                                <small className="truncate text-xs">: {recap.manager.nama_lengkap !== recap.sales_name ? recap.manager.nama_lengkap : "-"}</small>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <details className="group open:shadow-md border border-gray-200 rounded-xl overflow-hidden transition-all duration-300">
                                <summary className="list-none cursor-pointer">
                                    <div className="bg-gradient-to-r from-orange-50 to-orange-100 p-2 text-center">
